...
pub struct Activity120Episode {
    ...
    #[serde(rename = "id")]
    pub id: i32,
    #[serde(rename = "inactPaths")]
    pub inact_paths: String,
...
pub struct Activity120EpisodeTable {
    records: Vec<Activity120Episode>,
    by_id: HashMap<i32, usize>,
}

impl Activity120EpisodeTable {
    pub fn load(path: &str) -> Result<Self> {
        let json = std::fs::read_to_string(path)?;
        
        // Parse the [table_name, [records]] format
        let value: serde_json::Value = serde_json::from_str(&json)?;
        let records: Vec<Activity120Episode> = if let Some(array) = value.as_array() {
    ...
    #[inline]
    pub fn get(&self, id: i32) -> Option<&Activity120Episode> {
        self.by_id.get(&id).map(|&idx| &self.records[idx])
    }

    #[inline]
    pub fn all(&self) -> &[Activity120Episode] {
        &self.records
    }

    #[inline]
    pub fn iter(&self) -> std::slice::Iter<'_, Activity120Episode> {
        self.records.iter()
    }
