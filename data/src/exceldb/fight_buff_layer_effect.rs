...
pub struct FightBuffLayerEffect {
    ...
    #[serde(rename = "hideEffectWhenPlaying")]
    pub hide_effect_when_playing: i32,
    #[serde(rename = "id")]
    pub id: i32,
    #[serde(rename = "layer")]
    pub layer: i32,
...
pub struct FightBuffLayerEffectTable {
    records: Vec<FightBuffLayerEffect>,
    by_id: HashMap<i32, usize>,
}

impl FightBuffLayerEffectTable {
    pub fn load(path: &str) -> Result<Self> {
        let json = std::fs::read_to_string(path)?;
        
        // Parse the [table_name, [records]] format
        let value: serde_json::Value = serde_json::from_str(&json)?;
        let records: Vec<FightBuffLayerEffect> = if let Some(array) = value.as_array() {
    ...
    #[inline]
    pub fn get(&self, id: i32) -> Option<&FightBuffLayerEffect> {
        self.by_id.get(&id).map(|&idx| &self.records[idx])
    }

    #[inline]
    pub fn all(&self) -> &[FightBuffLayerEffect] {
        &self.records
    }

    #[inline]
    pub fn iter(&self) -> std::slice::Iter<'_, FightBuffLayerEffect> {
        self.records.iter()
    }
