...
pub struct GuideStepAdditionTable {
    records: Vec<GuideStepAddition>,
    by_id: HashMap<i32, usize>,
}

impl GuideStepAdditionTable {
    pub fn load(path: &str) -> Result<Self> {
        let json = std::fs::read_to_string(path)?;
        
        // Parse the [table_name, [records]] format
        let value: serde_json::Value = serde_json::from_str(&json)?;
        let records: Vec<GuideStepAddition> = if let Some(array) = value.as_array() {
    ...
    #[inline]
    pub fn get(&self, id: i32) -> Option<&GuideStepAddition> {
        self.by_id.get(&id).map(|&idx| &self.records[idx])
    }

    #[inline]
    pub fn all(&self) -> &[GuideStepAddition] {
        &self.records
    }

    #[inline]
    pub fn iter(&self) -> std::slice::Iter<'_, GuideStepAddition> {
        self.records.iter()
    }
