...
pub struct CharacterShopVoiceTable {
    records: Vec<CharacterShopVoice>,
}

impl CharacterShopVoiceTable {
    pub fn load(path: &str) -> Result<Self> {
        let json = std::fs::read_to_string(path)?;
        
        // Parse the [table_name, [records]] format
        let value: serde_json::Value = serde_json::from_str(&json)?;
        let records: Vec<CharacterShopVoice> = if let Some(array) = value.as_array() {
    ...
    #[inline]
    pub fn all(&self) -> &[CharacterShopVoice] {
        &self.records
    }

    #[inline]
    pub fn iter(&self) -> std::slice::Iter<'_, CharacterShopVoice> {
        self.records.iter()
    }
